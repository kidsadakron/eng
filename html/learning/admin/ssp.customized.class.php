...
class SSP {
    ...
    static function data_output ( $columns, $data, $isJoin = false )
    {
        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                $column = $columns[$j];
               // self::fatal( "An SQL error : ".$column['db']) ;
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        ...
        if($joinQuery){
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS DISTINCT ".implode(", ", $col)."
			 $joinQuery
			 $where
			 $extraWhere
			 $groupBy
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS DISTINCT `".implode("`, `", SSP::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            SSP::fatal( "An SQL error occurred: ".$e->getMessage().$sql.$bindings[0]['val']);
            //echo $sql.$bindings[0]['val'];
        }

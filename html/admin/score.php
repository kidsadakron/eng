...
ob_start();
	...
	$name =  $_SESSION["eln_name"];
	
	$id_number = $_SESSION["eln_id"]
?>
<script >
    ...
    success: function(data, textStatus, jqXHR){
           ...
           for(var i = 0 ; i < obj.length ; i++){
              line += '<tr >'; 
               if(i == 0){ 
                 line += '<td>#</td>';  
                 line += '<td><p>'+obj[i].unit_name+'</p></td>';
               }else{
                   line += '<td>'+(i)+'</td>';  
                   line += '<td><p >'+obj[i].unit_name+'</p></td>';
               }
               
               //href="index.php?tab=workUser&id=<?php echo $id_number; ?> &name='<?php echo $name; ?> '&unit='+(i)+'"
               if(i == 0){ 
                   if(obj[i].pertest == "-1"){
                ...
                if(obj[i].work == "-1"){
                   ...
                   line += '<td ><a href="index.php?tab=workUser&id=<?php echo $id_number; ?> &name=<?php echo $name; ?>&unit='+(i)+'">'+obj[i].work+'/10</a></td>';
               }
               if(i == 0){ 
                    if(obj[i].posttest == "-1"){
                        line += '<td style="background: khaki;">ยังไม่ได้ทดสอบ</td>';
...
<div  style="padding-top: 50px; top:0px">
    <h1><?php echo $name;?></h1>

...
$strSQL = "SELECT * FROM `eln`.`user` WHERE (`email` = '".$id_number."' OR id_number = '".$id_number."') AND `password` = '".$passOld."' ";
...
$objQuery = mysql_query($strSQL);
	...
	if(mysql_num_rows($objQuery) > 0 ){
			if($passNew == $passNewCon){
                            ...
                            if ($objQuery === TRUE) {
                                $strSQL = "SELECT * FROM `eln`.`user` WHERE (`email` = '".$data[0]["email"]."' OR id_number = '".$data[0]["id_number"]."') AND `password` = '".$passNewCon."' ";
                            mysql_query("SET NAMES utf8");
                         $objQuery = mysql_query($strSQL);

...
$strSQL = "SELECT * FROM `eln`.`user` WHERE (`email` = '".$data[0]["email"]."' OR id_number = '".$data[0]["id_number"]."') AND `password` = '".$data[0]["password"]."' ";
...
$objQuery = mysql_query($strSQL);
	...
	$bday = date_create_from_format('Y-m-d', $data[0]['bday']);
      ...
      <tbody id="score-tab">
       <tr>
        ...
        <tr>
          ...
          <form class="navbar-form navbar-left "  id="phone-form" autocomplete="off" style="display: none  " action="dist/updatePhone.php" method="post" target="iframe_target">
         ...
         <tr>
          ...
          <td style="width: 60%"><span id="bdayold"><?php echo $fbday; ?></span>
              <form class="navbar-form navbar-left "  id="bday-form" autocomplete="off" style="display: none  " action="dist/updateBday.php" method="post" target="iframe_target">
              
              <div>   <input  style="font-size: 22px;"  type="text"  data-date-format="dd-M-yyyy" name="bday" class="datepicker form-control" ></div>
